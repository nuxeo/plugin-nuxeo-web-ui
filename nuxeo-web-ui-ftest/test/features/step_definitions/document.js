...
Given(/^I have a document imported from file "(.+)"$/, function (mimeType) {
  return fixtures.documents.import(this.doc, fixtures.blobs.mimeTypeBlobs[mimeType]).then(d => this.doc = d);
});

Given(/^I have permission (\w+) for this document$/, function (permission) {
  return fixtures.documents.setPermissions(this.doc, permission, this.username).then((d) => { this.doc = d; });
});

Given(/^I have permission (\w+) for the document with path "(.+)"$/, function (permission, path) {
  return fixtures.documents.setPermissions(path, permission, this.username).then((d) => { this.doc = d; });
});
...
Given(/^This document has a (major|minor) version$/, function (versionType) {
  return fixtures.documents.createVersion(this.doc, versionType).then(d => this.doc = d);
});
...
When(/^I browse to the document$/, function () { this.ui.browser.browseTo(this.doc.path); });

When(/^I browse to the "(.*)" document page$/, function (page) {
  this.ui.browser.browseTo(`${this.doc.path}?p=${page}`);
});

When(/^I browse to the document with path "(.+)"$/, function (path) { this.ui.browser.browseTo(path); });
